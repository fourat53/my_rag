...
class AddTextResponse(BaseModel):
    status: str
    added_chunks: List[str] = Field(default_factory=list, description="Content of each added chunk")
    added_count: int = Field(description="Number of chunks added")


class DeleteTextResponse(BaseModel):
    status: str
    deleted_chunks: List[str] = Field(default_factory=list, description="Content of each deleted chunk")
...
class SearchResponse(BaseModel):
    ...
    chunks: List[ChunkSimilarityItem] = Field(default_factory=list, description="Top-k chunks by similarity")
